...
public static class AstraSdkInterop
{
    /// <summary>
    /// Credit to https://github.com/bibigone/AstraDotNetDemo for this method
    ...
    /// <summary>
    /// Although the the wrapper by Orbbec provides Astra.BodyTracking.SetLicense()
    /// it has a bug that causes crashes when the game stops playing
    /// use this wrapper instead
    /// </summary>
    /// <param name="licence"></param>
    /// <returns></returns>

    public static int SetLicenceKey(string licence)
    {
        return orbbec_body_tracking_set_license(Marshal.StringToHGlobalAuto(licence));
    }
    ...
    [DllImport("astra")]
    private static extern int orbbec_body_tracking_set_license(IntPtr licenseString);
}
